...
## Struttura del `Database` Universitario - `Entità`(tabelle):Departments,Degree Courses,Students,Courses,Teachers,Exam Sittings,Course-Teacher,Exam Enrollment&GradeRecords(8 in total).
...
`Dipartimenti` 1 a molti `Corsi_di_Laurea`
`Corsi_di_Laurea` 1 a molti `Corsi`
`Corsi_di_Laurea` 1 a molti `Studenti`
`Corsi` 1 a molti `Appelli_Esame`
`Corsi` molti a molti `Insegnati`
`Studenti` molti a molti `Appelli_Esame`